...
//------------------------------------------------------------------------------------
/*まとめ
$prefecture   都道府県+ナンバーの出力
$places       都道府県+説明文+画像の出力


if データが無い都道府県は、「観光スポット無し」

「検索」ボタンのaction="from.php"で1ページで完結する
洗濯された都道府県に関連する都道府県+説明文+画像をあげる
*/
    ...
    <form class="form-inline" action="inquiry.php" method="post">
      <div class="form-group">
        ...
        <?php foreach( $prefectures as $number => $prefecture ): ?>
          <option value="<?php print $number ?>"><?php print $prefecture; ?></option>
        <?php endforeach; ?>
        </select>
      </div>
      <button class="btn btn-primary btn-sm" type="submit"> 検 索 </button>
    </form>
    <p class="search-result">の観光スポットは 3 件見つかりました。</p>
    <div class="media">
